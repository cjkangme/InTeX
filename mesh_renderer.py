...
class Renderer(nn.Module):
    def __init__(self, device, opt):
        ...
        if opt.bg_image is not None and os.path.exists(opt.bg_image):
            ...
            bg_image = torch.from_numpy(bg_image.astype(np.float32) / 255).to(
                self.device
            )
            self.bg = (
                F.interpolate(
                    bg_image.permute(2, 0, 1).unsqueeze(0),
                    (opt.render_resolution, opt.render_resolution),
                    mode="bilinear",
                    align_corners=False,
                )[0]
                .permute(1, 2, 0)
    ...
    def load_mesh(self, path):
        ...
        self.mesh = Mesh.load(
            path,
            front_dir=self.opt.front_dir,
            retex=self.opt.retex,
            remesh=self.opt.remesh,
            device=self.device,
        )
    ...
    def render(self, pose, proj, h, w):
        ...
        # antialias
        albedo = (
            dr.antialias(albedo, rast, v_clip, self.mesh.f).squeeze(0).clamp(0, 1)
        )  # [H, W, 3]
        alpha = (
            dr.antialias(alpha, rast, v_clip, self.mesh.f).squeeze(0).clamp(0, 1)
        )  # [H, W, 3]
        ...
        if hasattr(self.mesh, "viewcos_cache"):
            viewcos_cache = dr.texture(
                self.mesh.viewcos_cache.unsqueeze(0),
                texc,
                uv_da=texc_db,
                filter_mode="linear-mipmap-linear",
            )
            viewcos_cache = dr.antialias(
                viewcos_cache, rast, v_clip, self.mesh.f
            ).squeeze(
                0
            )  # [H, W, 3]
        ...
        if hasattr(self.mesh, "ori_albedo"):
            ori_albedo = dr.texture(
                self.mesh.ori_albedo.unsqueeze(0),
                texc,
                uv_da=texc_db,
                filter_mode="linear-mipmap-linear",
            )
            ori_albedo = dr.antialias(ori_albedo, rast, v_clip, self.mesh.f).squeeze(
                0
            )  # [H, W, 3]
