...
def mipmap_linear_grid_put_2d(
    ...
    while min(cur_H, cur_W) > min_resolution:
        ...
        cur_result, cur_count = linear_grid_put_2d(
            cur_H, cur_W, coords, values, return_count=True
        )
        result[mask] = (
            result[mask]
            + F.interpolate(
                cur_result.permute(2, 0, 1).unsqueeze(0).contiguous(),
                (H, W),
                mode="bilinear",
                align_corners=False,
            )
        ...
        count[mask] = (
            count[mask]
            + F.interpolate(
                cur_count.view(1, 1, cur_H, cur_W),
                (H, W),
                mode="bilinear",
                align_corners=False,
            ).view(H, W, 1)[mask]
...
def nearest_grid_put_3d(H, W, D, coords, values, return_count=False):
    ...
    result = torch.zeros(
        H, W, D, C, device=values.device, dtype=values.dtype
    )  # [H, W, C]
    count = torch.zeros(
        H, W, D, 1, device=values.device, dtype=values.dtype
    )  # [H, W, 1]
    weights = torch.ones_like(values[..., :1])  # [N, 1]
...
def linear_grid_put_3d(H, W, D, coords, values, return_count=False):
    ...
    indices_001 = indices_000 + torch.tensor(
        [0, 0, 1], dtype=torch.long, device=indices.device
    )
    indices_010 = indices_000 + torch.tensor(
        [0, 1, 0], dtype=torch.long, device=indices.device
    )
    indices_011 = indices_000 + torch.tensor(
        [0, 1, 1], dtype=torch.long, device=indices.device
    )
    indices_100 = indices_000 + torch.tensor(
        [1, 0, 0], dtype=torch.long, device=indices.device
    )
    indices_101 = indices_000 + torch.tensor(
        [1, 0, 1], dtype=torch.long, device=indices.device
    )
    indices_110 = indices_000 + torch.tensor(
        [1, 1, 0], dtype=torch.long, device=indices.device
    )
    indices_111 = indices_000 + torch.tensor(
        [1, 1, 1], dtype=torch.long, device=indices.device
    )
    ...
    result, count = scatter_add_nd_with_count(
        ...
        count,
        indices_000,
        values * w_000.unsqueeze(1),
        weights * w_000.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_001,
        values * w_001.unsqueeze(1),
        weights * w_001.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_010,
        values * w_010.unsqueeze(1),
        weights * w_010.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_011,
        values * w_011.unsqueeze(1),
        weights * w_011.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_100,
        values * w_100.unsqueeze(1),
        weights * w_100.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_101,
        values * w_101.unsqueeze(1),
        weights * w_101.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_110,
        values * w_110.unsqueeze(1),
        weights * w_110.unsqueeze(1),
    )
    result, count = scatter_add_nd_with_count(
        result,
        count,
        indices_111,
        values * w_111.unsqueeze(1),
        weights * w_111.unsqueeze(1),
    )
...
def mipmap_linear_grid_put_3d(
    H, W, D, coords, values, min_resolution=32, return_count=False
):
    # coords: [N, 3], float in [-1, 1]
    # values: [N, C]
    ...
    while min(min(cur_H, cur_W), cur_D) > min_resolution:
        ...
        cur_result, cur_count = linear_grid_put_3d(
            cur_H, cur_W, cur_D, coords, values, return_count=True
        )
        result[mask] = (
            result[mask]
            + F.interpolate(
        ...
        count[mask] = (
            count[mask]
            + F.interpolate(
                cur_count.view(1, 1, cur_H, cur_W, cur_D),
                (H, W, D),
                mode="trilinear",
                align_corners=False,
            ).view(H, W, D, 1)[mask]
...
def grid_put(
    shape, coords, values, mode="linear-mipmap", min_resolution=32, return_raw=False
):
    # shape: [D], list/tuple
    ...
    if mode == "nearest":
        ...
        if D == 2:
            return mipmap_linear_grid_put_2d(
                *shape, coords, values, min_resolution, return_raw
            )
        else:
            return mipmap_linear_grid_put_3d(
                *shape, coords, values, min_resolution, return_raw
            )
    else:
